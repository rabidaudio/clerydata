...
db.getConnection(function(err, connection){
    ...
    function(error, results) {
        ...
        fuse = new Fuse(results, {
            ...
            threshold: 0.4,
            id: "id",
            //this is neccessary because for some reason the default getFn won't return integer ids:
            getFn: function(item, key){ return item[key]; }
        });
...
/**
    Return an array of school id's with similar names. Doing it this way not only improves the text search functionality over
    the database's capability, but also significantly speeds up queries by limiting the search to matching names
*/
function fuzzy_search_name(name){ return _.flatten(fuse.search(name)); }

/**
    This object makes database results less flat
...
Schools.search = function(params, callback){
    ...
    if(params.location){
        _.each(params.location, function(value, key, list){
            if( ["address", "city", "state", "zip"].indexOf(key) < 0 ) return; //todo maybe throw error
            sql += " AND "+db.escapeId(key)+(typeof(value) === 'string' ? " LIKE " : " = ")+db.escape(value);
        });
    }

    //this is the stuff to do first (name search)
    if(params.name){
        var possible_schools = fuzzy_search_name(params.name);
...
Schools.get = function(id, callback){
    db.getConnection(function(err, connection){
        connection.query("SELECT * FROM schools WHERE id = ?", [id], function(error, result){
            //hanlde error
            connection.release();
            if(callback instanceof Function) callback( result.length>0 ? new School(result[0]) : []);
        });
    });
};
